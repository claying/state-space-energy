...
x_train = pd.concat((x_train, x_train.shift(1)), axis=1).iloc[1:,:]
y_train = y_train.iloc[1:,:]
x_train, y_train, x_test, y_test = split_data(x_train, y_train, 0.76)
...
# y_train = pd.concat((y_train, y_train.shift(1)), axis=1).iloc[1:,:]
...
model = kl.em(y_train, x_train, em_vars=em_vars, n_iter=100)
...
y_pred, pred_upper, pred_lower = model.forecast(np.vstack((x_train[-1,:], x_test)), x0, P0)
print y_pred.shape
print y_test.shape
y_pred = np.maximum(y_pred, 0)
# y_pred = y_pred[:, :(y_dim/2)]
